...
raw_flow_path_fastflowkitti_valid = config["raw_flow_path_fastflowkitti"][1]
labels_valid_path = config["labels_valid_path_fastflowkitti_2"]
sampling_frac = config["sampling_frac_valid"]

with open(labels_valid_path) as csvf:
...
if __name__ == "__main__":
    ...
    for i, label in enumerate(labels):
        ...
        curr_imgpath = curr_imgpath.replace(
            "flow_valid_2", "raw_with_flow_valid")
        ...
        if curr_imgpath != prev_imgpath:
            reg = re.search(
                rf"{raw_flow_path_fastflowkitti_valid}/(.+).png", prev_imgpath)
            imgname = reg.group(1)
            reg = re.search(r"seq_([0-9]*)_img_(.+)", imgname)
            idx = int((int(reg.group(1)) - 1) / 20)
            plt.savefig(
                f"{visual_path}/{imgname}.png", bbox_inches="tight", pad_inches=0)
            plt.clf()
